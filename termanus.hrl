...
%% record fields
-define(record_field, {record_field, _, _}). %% {record_field,ANNO,Rep(A)}.
-define(record_field_exp, {record_field, _, _, _}). %% {record_field,ANNO,Rep(A),Rep(E)}.
-define(typed_record_field, {typed_record_field, ?record_field, _}). %% {typed_record_field,{record_field,ANNO,Rep(A)},Rep(T)}.
-define(typed_record_field_exp, {typed_record_field, ?record_field_exp, _}). %% {typed_record_field,{record_field,ANNO,Rep(A),Rep(E)},Rep(T)}.
...
-record(tuple, {anno, fields}). %% {tuple,ANNO,[Rep(P_1), ..., Rep(P_k)]}.
%% {record,ANNO,Rep(E_0),Name,[{record_field,ANNO,Rep(Field_1),Rep(E_1)}]}.
...
-define(map_update, {map, _, _, _}).
-record('receive', {anno, val}). %% {'receive',ANNO,[Rep(Cc_1)]}.
-define(receive_after, {'receive', _, _, _, _}).
-record('receive', {anno, body, exp, val1, val2}). %% {'try',ANNO,Rep(B),[Rep(Cc_1)],[Rep(Tc_1)],Rep(A)}.
...
%% {record_field,ANNO,Rep(Gt_0),Name,Rep(Field)}.
%% {record_index,ANNO,Name,Rep(Field)}.
...
%% {op,ANNO,Op,Rep(T_1),Rep(T_2)}.
%% {op,ANNO,Op,Rep(T_0)}.
%% {type,ANNO,N,[Rep(T_1)]}.
%% {type,ANNO,record,[Rep(Name),Rep(F_1)]}. 
%% {remote_type,ANNO,[Rep(M),Rep(N),[Rep(T_1)]]}.
%% {type,ANNO,tuple,any}.
