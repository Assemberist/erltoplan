...
parse(Element = #remote{mod = #atom{val = gen_server}, func = #atom{}}, Caller) ->
...
parse(#remote{mod = #atom{val = Far}, func = #atom{val = FarFun}}, Caller = {_, #function{name = Fun}})
	...
	erlout:put(gs_links, [{Caller, {Far#atom.val, FarFun#atom.val}}]);

parse(Element = #call{who = #atom{val = Fun}}, Caller = {Mod, #function{name = Fun}})
            when Fun == handle_call; Fun == handle_cast ->
